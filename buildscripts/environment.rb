...
namespace :env do
  ...
  task :set_dirs do
    ...
    FOLDERS[:peds][:out] = File.join(FOLDERS[:src], PROJECTS[:peds][:dir], 'bin', CONFIGURATION)
    CLEAN.include(FOLDERS[:peds][:out])

    # for tests
    FOLDERS[:peds][:test_out] = File.join(FOLDERS[:src], PROJECTS[:peds][:test_dir], 'bin', CONFIGURATION)
    FILES[:peds][:test] = File.join(FOLDERS[:peds][:test_out], "#{PROJECTS[:peds][:test_dir]}.dll")
  ...
  desc "set 'dev' environment target"
  task :dev do
    ENV['ENV_TARGET'] = ENV_TARGET = 'dev'
	ENV['ENV_NETWORKSHAREHOST'] = ENV_NETWORKSHAREHOST = 'SEPITAB01'
	ENV['ENV_NETWORKSHAREUSERNAME'] = ENV_NETWORKSHAREUSERNAME = 'nikkan.SEPITAB01'
	ENV['ENV_NETWORKSHAREPASSWORD'] = ENV_NETWORKSHAREPASSWORD = '88v63IEO'
